...
float FDelaunaySurfaceParams::SampleCurve(int32 Index, float Time) const
{
...
float FDelaunaySurfaceParams::SampleCurves(float X, float Y) const
{
...
FDelaunayMaterialParams::FDelaunayMaterialParams()
:	ProjectUV(false)
{
}

FDelaunayInstanceParams::FDelaunayInstanceParams()
...
FVector FDelaunayInstanceParams::SampleAlignment(FRandomStream& Random)
{
...
float FDelaunayInstanceParams::SampleRotation(FRandomStream& Random)
{
...
FDelaunayHoleParams::FDelaunayHoleParams()
:	Radius(100.0f)
{
}
...
void UDelaunayFillSplineLibrary::GenerateDelaunay(
	...
	TArray<FTransform>& Transforms)
{
	if (IsValid(Left) && IsValid(Right) && Surface.FillerMaxSize >= SMALL_NUMBER)
	{
		...
		*/
		...
		FTriangulation2D Triangulation2D;
		...
		*/
		...
		for (int32 Index = 0; Index < TriangleMesh.Triangulation.Triangles.Num(); Index++)
		{
			...
			if (Triangle.Enabled)
			{
				...
				const FVector Normal = (AC ^ AB).GetSafeNormal();
		...
		for (int32 Index = 0; Index < AdjPoints.Num(); Index++)
		{
			...
			if (N > 1)
			{
				...
				const FVector Principal = FMatrix3x3(Deltas).PowerMethod(Tangent, 16);
				const FVector Residual = (Principal ^ Normal).GetSafeNormal();

				FTransform Basis;
				Basis.SetLocation(Center);


				const FVector TileUpVector = Instances.SampleAlignment(RandomStream).GetSafeNormal();
				if (TileUpVector.SizeSquared() > SMALL_NUMBER)
				{
					...
					const FVector Axis = (Quat * TileRightVector).GetSafeNormal();
					...
					{
						const FVector Forward = ((Tangent ^ Normal) ^ Normal).GetSafeNormal();
