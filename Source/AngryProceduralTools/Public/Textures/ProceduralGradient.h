...
#include "ProceduralGradient.generated.h"

#define MAX_ARRAY_SLICES 512

class UProceduralGradient;

/** Represents a 2D Texture Array to the renderer. */
class FProceduralGradientResource : public FTextureResource
{
public:

	FProceduralGradientResource() : Owner(nullptr) {}
	FProceduralGradientResource(UProceduralGradient* InOwner);
	virtual ~FProceduralGradientResource();
...
UCLASS()
class ANGRYPROCEDURALTOOLS_API UProceduralGradient : public UTexture2DDynamic
{
...
public:
	...
	*/
	 ...
	 */
	UFUNCTION()
		virtual void GenerateGradient(int32 GradientSizeX, int32 GradientSizeY, TArray<FLinearColor>& Pixels);

	/** Pixel cache used by resource */
	UPROPERTY(Transient)
		TArray<uint32> DataCache;

	UPROPERTY(EditAnywhere, Category = "Procedural", meta = (ClampMin = 1, ClampMax = 10))
		int32 WidthPow2;
